...
```
faca um expose do deployment com
    k expose deployments.apps nginx
...
# Locust 
yamls retirados do https://github.com/badtuxx/giropops-senhas-labs/tree/main/locust

apos deployar o configmap/deployment/service do locust faça um port-forward para acessar a pagina do locust
    kubectl port-forward services/locust-giropops 8080:80

Coloque como quantidades de usuarios 1000
Spawn 100
e host o padrão (nomedoservice.namespace.type.cluster.local) ficando
http://nginx.default.svc.cluster.local

acompanhe no as requisicoes em algum pod com
    kubectl logs -f nginx-XXXXX
e veja em outra aba o pod sendo scalado com o get pods e com o describe do hpa
...
desiredReplicas=⌈currentReplicas×(currentMetricValuedesiredMetricValue)⌉
...
Exemplos com Valores Específicos:
    ...
    Exemplo de Escala para Cima:
        ...
        Cálculo: ⌈2×(80%/50%)⌉=⌈3.2⌉=4⌈2×(80%/50%)⌉=⌈3.2⌉=4 réplicas

    Exemplo de Escala para Baixo:
        Réplicas atuais: 5
        Valor atual da métrica (CPU): 30%
        Valor desejado da métrica (CPU): 50%
        Cálculo: ⌈5×(30%/50%)⌉=⌈3⌉=3⌈5×(30%/50%)⌉=⌈3⌉=3 réplicas

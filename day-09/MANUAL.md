...
## Instalando Ingress Controller nos Providers
### Minikube
...
## Suba o app o Redis e os seus respectivos services
...
Após subir todos execute o port forward para testar no browser
...
### OBS
Repare que ele irá quebrar pq yaml acima tudo que começar com /giropops-senhas será redirecionado para o / da aplicaçao, portanto ele nao irá carregar os outros arquivos de static e css da aplicacao

para corrigir isso pode se criar outro yaml com o path na /
...
metadata:
  name: giropops-senhas
  annotations:
    nginx.ingress.kubernetes.io/rewrite-target: /  # no container ele vai jogar nesse /
spec:
  ingressClassName: nginx
  rules:
  - http:
      paths:
      - path: / # caminho que vai ser acessado externamente
        pathType: Prefix
        backend:
          service:
            name: giropops-senhas
...
com isso ao bater na url o localhost, ele deverá funcionar normalmente.
porém nao conseguiremos ter outra aplicacao, para isso usamos o host como nesse outro exemplo
só criar outro ingress
...
spec:
  ingressClassName: nginx
  rules:
  - host: giropops-senhas.local # colocar no /etc/hosts(windows) para resolver esse dominio "127.0.0.1 giropops-senhas.local"
    http:
      paths:
      - path: / # caminho que vai ser acessado externamente
        pathType: Prefix

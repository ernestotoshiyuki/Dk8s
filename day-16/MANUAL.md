...
## O que sao as Taints
Basicamente são manchas que colocamos somente nos nodes, como chave e valor seguidos por ":" que podem ser do tipo:
NoSchedule (a partir daquele ponto, nao recebe mais nenhum novo pod - geralmente no control plane)
NoExecute (para nao subir nenhum pod nesse node - ideal para manutencao do node)

exemplo
```bash
kubectl taint node "nomedonode-worker" manutencao=true:NoExecute

# logo aplicando esse comando os pods são automaticamente migrados para outro node o que pode ser verificado com 
kubectl get pods -o wide

# para remover a taint só colocar um "-"no final 
kubectl taint node "nomedonode-worker" manutencao=true:NoExecute-
```
...
```yaml
        # o que foi testado e validado é assim
        affinity:
                podAntiAffinity:
                requiredDuringSchedulingIgnoredDuringExecution:
                - labelSelector:    # labelSelector é usado para selecionar os pods que devem ser evitados
                matchLabels:
                        app: nginx
                topologyKey: "region" 
```
